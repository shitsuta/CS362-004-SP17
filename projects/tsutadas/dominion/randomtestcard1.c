...
int main() {
	...
	for (i = 0; i < NUMTEST; ++i)
	{
		...
		r = cardEffect(village, 0, 0, 0, testG, 0, 0);

		//Check State Change
		originalAssert("Error on handCount", originalG->handCount[thisPlayer] == testG->handCount[thisPlayer], &bugs);
		originalAssert("Error on numActions",originalG->numActions - 1 + 2 == testG->numActions, &bugs);
